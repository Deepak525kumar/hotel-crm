 ...
 * `/rework/[id]`, neither of which existed there. Nothing caught it —
 * expo-router resolves paths as strings at runtime, so typecheck is blind to
 * it and no test renders a tap.
 *
 * It caught one here on its first run, 2026-09-22: the More menu offered
 * Archive and pushed to `/admin/archive`, which did not exist. An admin
 * would have tapped a menu row and had nothing happen at all.
 */
