...
function prefetchOwnPhoto(user: User, accessToken: string): void {
  ...
  // files running under Jest's `node` test environment -- expo-image's own
  // import chain touches Expo's runtime setup, which references `__DEV__`
  // and crashes there. The early return above means this line is only
  // reached with a real photo-bearing user, so it never executes in those
  // suites' mocked responses.
