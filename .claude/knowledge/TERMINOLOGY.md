...
| Job request | Named backend module (`backend-job-requests`) and Prisma domain concept (`JobRequest` model, `@@map("WorkRequest")` — physical table name unchanged); a manager-created request for one or more workers on a shift, published to the eligible hotel roster. Renamed from "Work request"/`WorkRequest` by Epic 9 PR 9.4 (TREQ-013/MIG-GAP-12, re-label off "marketplace" framing) — approved promotion per `SPEC-JOB-DISPATCH-001`'s own Proposed Knowledge Delta (`MODULE_SPEC.md` §"Proposed Knowledge Deltas"), applied in this PR per its own text, not deferred. The public route path `/work-requests` is retained unchanged (not renamed). | `backend/src/modules/job-requests/`, `backend/prisma/schema.prisma`, `SPEC-JOB-DISPATCH-001@0.3.3` | Canonical (promoted by Epic 9 PR 9.4; see SYNC-061) |
...
| Broadcast (Job Request) | `[TARGET]` fallback dispatch mechanism: a manager raises a standalone `JobRequest` naming one or more skills and a headcount per skill (e.g. "2 Cleaners + 1 Waiter"); only workers with a matching skill who are free that day are eligible, notified, and may accept — first accept wins each skill slot, later responders receive "requirement fulfilled." An unfilled broadcast auto-closes 6 hours after creation (manager may close manually sooner). Fires only on an explicit standalone request — never on a calendar edit (distinct from the Primary/calendar direct-assignment path, `SPEC-JOB-DISPATCH-001` `TREQ-001`). Persistence (`raiseBroadcast()`, `JobRequestSkillSlot`) and eligibility computation (`getBroadcastEligibility()`) built by Epic 9 PR 9.7 (`TREQ-002`/`TREQ-003`); notification delivery (`enqueueBroadcastNotifications()`, `NotificationType.JOB_REQUEST_BROADCAST`) built by Epic 9 PR 9.8 (`TREQ-003` delivery half); first-accept arbitration + "requirement fulfilled" (`acceptBroadcast()`) built by Epic 9 PR 9.9 (`TREQ-004`/`TREQ-005`); 6h auto-close + manual close (`JobRequestAutoCloseJob`, `manualClose()`, `NotificationType.JOB_REQUEST_CLOSED`) built by Epic 9 PR 9.10 (`TREQ-006`) — every `TREQ`/`MIG-GAP` row this capability's target-state scope named is now built in code. | `CONFIRMED_REQUIREMENTS_REGISTER.md` §13 (Fallback), `PIVOT_DESIGN_DOCUMENT.md` §4.4/§7.3, `SPEC-JOB-DISPATCH-001@0.3.8` | Canonical (promoted by Epic 9 PR 9.7, per `SPEC-JOB-DISPATCH-001`'s own Proposed Knowledge Delta; see SYNC-064, SYNC-066, SYNC-067, SYNC-068, SYNC-069) |
| Skill | The `SkillTag` Prisma enum (`CLEANER`/`PUBLIC_SERVICE`/`KITCHEN_DISHWASHER`/`WAITER`) — a worker's fixed capability set (`EmploymentRecord.skills`, PR 5.6) and, as of Epic 9 PR 9.7, the unit a broadcast `JobRequest` requests headcount against (`JobRequestSkillSlot.skill`). Replaces free-text `position` for the broadcast path only; the pre-existing marketplace `JobRequest.position` field is untouched (`TREQ-010`, `MIG-GAP-05`). | `backend/prisma/schema.prisma` (`SkillTag` enum, `EmploymentRecord.skills`, `JobRequestSkillSlot.skill`), `CONFIRMED_REQUIREMENTS_REGISTER.md` §4, `SPEC-JOB-DISPATCH-001@0.3.4` | Canonical (promoted by Epic 9 PR 9.7, per `SPEC-JOB-DISPATCH-001`'s own Proposed Knowledge Delta; see SYNC-064) |
| Slot | One unit of headcount capacity for a single skill on a broadcast `JobRequest` (e.g. "2 Cleaners" is a 2-headcount `CLEANER` slot). Modeled by `JobRequestSkillSlot` (`skill`, `headcount`, `confirmed_count`), added Epic 9 PR 9.7. Claimed via first-accept optimistic concurrency (`acceptBroadcast()`, a transactional conditional `updateMany` on `confirmed_count`, `ADR-057` — no separate `version` column needed at this per-skill grain, per `ADR-057`'s 2026-07-30 addendum) — built Epic 9 PR 9.9. On a filled slot, a later claimant receives "requirement fulfilled," not an error. | `backend/prisma/schema.prisma` (`JobRequestSkillSlot`), `PIVOT_DESIGN_DOCUMENT.md` §5.5/§7.3, `ADR-057`, `SPEC-JOB-DISPATCH-001@0.3.6` | Canonical (promoted by Epic 9 PR 9.9, per `SPEC-JOB-DISPATCH-001`'s own Proposed Knowledge Delta; see SYNC-067) |
