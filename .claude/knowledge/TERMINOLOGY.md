...
| Broadcast (Job Request) | `[TARGET]` fallback dispatch mechanism: a manager raises a standalone `JobRequest` naming one or more skills and a headcount per skill (e.g. "2 Cleaners + 1 Waiter"); only workers with a matching skill who are free that day are eligible, notified, and may accept — first accept wins each skill slot, later responders receive "requirement fulfilled." Fires only on an explicit standalone request — never on a calendar edit (distinct from the Primary/calendar direct-assignment path, `SPEC-JOB-DISPATCH-001` `TREQ-001`). Persistence (`raiseBroadcast()`, `JobRequestSkillSlot`) and eligibility computation (`getBroadcastEligibility()`) built by Epic 9 PR 9.7 (`TREQ-002`/`TREQ-003`); notification delivery (`enqueueBroadcastNotifications()`, `NotificationType.JOB_REQUEST_BROADCAST`) built by Epic 9 PR 9.8 (`TREQ-003` delivery half); first-accept arbitration + "requirement fulfilled" (`acceptBroadcast()`) built by Epic 9 PR 9.9 (`TREQ-004`/`TREQ-005`); auto-close remains unbuilt (PR 9.10). | `CONFIRMED_REQUIREMENTS_REGISTER.md` §13 (Fallback), `PIVOT_DESIGN_DOCUMENT.md` §4.4/§7.3, `SPEC-JOB-DISPATCH-001@0.3.6` | Canonical (promoted by Epic 9 PR 9.7, per `SPEC-JOB-DISPATCH-001`'s own Proposed Knowledge Delta; see SYNC-064, SYNC-066, SYNC-067) |
