...
5. Record knowledge/index changes and revision-bound synchronization state. The Context Management Layer needs no explicit prune: it holds no persistent state, so each `context-loader.js resolve` is minimal by construction against the terminal revision ([CONTEXT_ARTIFACTS.md](../constitution/CONTEXT_ARTIFACTS.md) §8). When a boot document changed this cycle, refresh the Boot Context digest key in `../knowledge/SYNC_STATE.yaml` `cache_state.boot_context` (the `resolve` output prints the current `boot_cache_key`).
