...
## 8. Context Management Layer (framework 1.5.0)

**Status:** Proposed — pending human ratification (Engineering Constitution §20), consistent with the ADR-001..010 precedent and the §6–§7 (1.2.0) precedent. Additive; changes no gate, finding schema, confidence rule, loop bound, or specialist boundary. See [ADR-020](../../docs/14-governance/architecture-decisions/ADR-020-context-management-layer.md).

Sections 1–7 define the reusable Context Artifacts and their lifecycle. They did **not**, before 1.5.0, say *which* artifacts a given workflow must load — that was left to each workflow's prose, so the "minimum context package" (CLAUDE.md §Context Assembly) was re-derived per workflow. The **Context Management Layer** operationalizes §1–§7 as a single, deterministic, executable mechanism without adding policy.

- **Declarative load plan (JSON).** [`../context/BOOT_MANIFEST.json`](../context/BOOT_MANIFEST.json), [`../context/EXECUTION_MANIFEST.json`](../context/EXECUTION_MANIFEST.json), and [`../context/ARTIFACT_DEPENDENCIES.json`](../context/ARTIFACT_DEPENDENCIES.json) are the machine-readable projection of, respectively, the Boot Context set (§2.1), the per-workflow minimum artifact set, and the dependency graph + cache scopes (§2, §3.1). They **reference** the artifact IDs and document paths defined here; they restate no policy text — in particular the invalidation *semantics* remain solely in §3.2 and the *version* authority solely in [`../VERSION.yaml`](../VERSION.yaml) (the manifests carry neither). JSON so the loader needs no parser. On any conflict this document wins. The per-workflow document/artifact set is the one piece of information not derivable from existing metadata without parsing prose (rejected on determinism grounds, ADR-020); everything derivable is omitted (the workflow file is derived as `.claude/workflows/<id>.md`; gate planning stays with the workflow / `LOOP_REGISTRY.yaml`).
- **Single engine.** [`../tooling/context-loader.js`](../tooling/context-loader.js) is the only implementation of the loading behaviour. Given a workflow it resolves the minimum document + artifact set (deterministic loading), expands artifact dependencies (§2), reuses warm Repository-Session artifacts by reference (§6, §1.3), and reports the **load delta as the cold set** — the artifacts not already warm for the current revision. No workflow restates loading logic.
- **No persistent generated state.** Warm/cold and the delta are **derived at call time** from the authoritative [`SESSION_STATE.yaml`](../knowledge/SESSION_STATE.yaml) (§6) and [`SYNC_STATE.yaml`](../knowledge/SYNC_STATE.yaml) `cache_state` (§3.3); the layer writes no checkpoint files, so there is nothing to store, rebuild, or prune (§1.5). Revision-scoped artifacts are warm iff the session's `baseline_revision` matches; the framework-version-scoped **Boot Context is keyed by a content digest of its boot documents** (a blob-digest form §3.1 already permits), so a framework-version bump that changes no boot document does not invalidate it — reducing invalidation scope versus a version-string key.
- **Automatic inheritance.** Every workflow enters through Pre-flight, which resolves its load set from this layer ([preflight.md](../workflows/preflight.md) §Context Artifact Reuse). Because the layer holds no persistent state, a revision change ([repository-synchronization](../workflows/repository-synchronization.md)) rebuilds the execution context automatically on the next resolve, and there is nothing to prune at [postflight](../workflows/postflight.md) — the behaviour is inherited platform-wide with no per-workflow duplication and no manual upkeep.
- **Enforcement (correctness only).** `context-loader.js --validate` is a deterministic sub-check owned by the Consistency Reviewer wherever consistency review applies (G4), reused at G6/G9, and independently enforced by CI ([REVIEW_GATES.md](REVIEW_GATES.md) Applicability Rules) — the same dual-enforcement pattern as Repository Integrity Validation (ADR-019). CI validates correctness only; the reduction figure (`--measure`) is a labelled **estimate** (a byte proxy, not a tokenizer count) and is not run in blocking CI.
- **Fail-safe (§1.5).** Absent the manifests or the loader, the platform degrades to pre-1.5.0 behaviour — read every authoritative document. The layer can only reduce what is loaded; it never gates correctness.
