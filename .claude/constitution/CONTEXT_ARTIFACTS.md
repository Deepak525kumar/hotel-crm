...
- **Declarative load plan.** [`../context/BOOT_MANIFEST.yaml`](../context/BOOT_MANIFEST.yaml), [`../context/EXECUTION_MANIFEST.yaml`](../context/EXECUTION_MANIFEST.yaml), and [`../context/ARTIFACT_DEPENDENCIES.yaml`](../context/ARTIFACT_DEPENDENCIES.yaml) are the machine-readable projection of, respectively, the Boot Context set (§2.1), the per-workflow minimum artifact set, and the dependency/invalidation graph (§2, §3.2). They **reference** the artifact IDs and document paths defined here and elsewhere; they restate no policy text. On any conflict this document wins.
- **Single engine.** [`../tooling/context-loader.js`](../tooling/context-loader.js) is the only implementation of the loading behaviour. Given a workflow it resolves the minimum document + artifact set (deterministic loading), expands artifact dependencies (§2), reuses warm Repository-Session artifacts by reference (§6, §1.3), loads only the delta since the last checkpoint, applies revision-aware validity and the §3.2 invalidation bindings, and can checkpoint (session state) and prune. No workflow restates loading logic.
- **Generated, non-authoritative projections.** The loader emits `../context/SESSION_CONTEXT.yaml` and `../context/CACHE_STATE.yaml` as advisory, `.gitignore`d checkpoints — projections of the authoritative [`SESSION_STATE.yaml`](../knowledge/SESSION_STATE.yaml) (§6) and [`SYNC_STATE.yaml`](../knowledge/SYNC_STATE.yaml) `cache_state` (§3.3). They hold no unique authority; deleting them changes nothing (§1.5).
- **Automatic inheritance.** Every workflow enters through Pre-flight, which resolves its load set from this layer ([preflight.md](../workflows/preflight.md) §Context Artifact Reuse); [repository-synchronization](../workflows/repository-synchronization.md) rebuilds the checkpoint on revision rebind, and [postflight](../workflows/postflight.md) prunes it — so the behaviour is inherited platform-wide with no per-workflow duplication.
- **Enforcement.** `context-loader.js --validate` is a deterministic sub-check owned by the Consistency Reviewer wherever consistency review applies (G4), reused at G6/G9, and independently enforced by CI ([REVIEW_GATES.md](REVIEW_GATES.md) Applicability Rules) — the same dual-enforcement pattern as Repository Integrity Validation (ADR-019).
