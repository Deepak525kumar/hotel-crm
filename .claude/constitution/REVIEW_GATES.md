...
- **Context Manifest Validation:** a mandatory, deterministic sub-check owned by the Consistency Reviewer wherever consistency review applies to any change touching the Context Management Layer (`../context/*`, `../tooling/context-loader.js`, workflow files, or `../VERSION.yaml`); never a separate gate or workflow. Reused unchanged at G6 and G9. Confirms every manifest reference resolves, every `.claude/workflows/*.md` has an EXECUTION_MANIFEST entry, the manifest framework version matches `../VERSION.yaml`, and every `invalidated_by` trigger exists in `../knowledge/SYNC_STATE.yaml` `context_invalidators`, via the single implementation at [`../tooling/context-loader.js`](../tooling/context-loader.js) (`--validate`). `NOT_APPLICABLE` when the change touches none of those paths. Independently and unconditionally enforced on every pull request by CI (`../../.github/workflows/ci.yml`, job `context-manifest`) regardless of authorship — the same human-and-AI-symmetric backstop pattern as Repository Integrity Validation (ADR-019); the framework gate reuses CI's exact output rather than re-deriving it. See [CONTEXT_ARTIFACTS.md](CONTEXT_ARTIFACTS.md) §8 and [ADR-020](../../docs/14-governance/architecture-decisions/ADR-020-context-management-layer.md).
