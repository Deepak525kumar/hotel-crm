...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-AUTH-001 / 0.1.0` |
| Status | `REVIEW` |
| Owner | `unassigned (SYNC-001, human authority required)` |
| Authors / reviewers | Author: Module Author agent. Reviewers: not yet run (this is the initial candidate submitted for G4 architecture/dependency/consistency/security/performance review). |
...
- `backend/src/middleware/permissions.ts` (`requireRole`, `requirePermission`,
  ...
  `DEPENDENCY_GRAPH.yaml:388-399`, does not include `backend-auth`).
...
| `TREQ-AUTH-002` Regional Manager sees all Hotel-Manager-visible data across every hotel in their managed Hotel Group; a normal Hotel Manager is scoped to one hotel; Admin is system-wide | CONFIRMED §1:20-22; PIVOT §5.4:173-179 | Confirmed authority | Target; unbuilt (`MIG-GAP-AUTH-02`) |
...
| `TREQ-AUTH-008` no login rate-limiting, no CAPTCHA; rate limiting is handled at the Nginx/Cloudflare edge, outside this module | CONFIRMED §2:34-35; PIVOT §5.2:160,163 | Confirmed authority | Target; **already aligned with current state** (`REQ-AUTH-021`) — not a gap |
...
| Role (current) | One of 4 lowercased string tokens (`worker`,`checker`,`manager`,`admin`) carried on the access-token `role` claim and matched exactly by `requireRole`. `[TARGET]` grows to 5 tokens including `regional_manager` (exact casing UNSTATED — `OQ-AUTH-13`). | `middleware/permissions.ts:65-94`; CONFIRMED §1 |
| Permission | A granular string code (e.g. `"hotels:read"`, `"admin:*"`) assigned at signup from `ROLE_PERMISSIONS[role]` and carried on the access token; matched exactly or via `resource:*` wildcard by `requirePermission`. | `config/constants.ts:88-129`; `middleware/permissions.ts:7-63` |
| Scope | `[TARGET]` the hotel or hotel-group a user may act within, carried on the access token alongside `user_id`/`role` so authorization checks avoid a DB round trip; NOT present in any current token. | PIVOT §5.3:170 |
...
| REQ-AUTH-021 | No rate-limiting or CAPTCHA exists in this module or anywhere in the backend. | Must (confirmed non-goal, already aligned) | Repeated login/signup/password-reset attempts from the same caller are never throttled or challenged at the application layer; `RATE_LIMIT_EXCEEDED`/`TOO_MANY_REQUESTS` constants exist but are never thrown. | — |
...
| TREQ-AUTH-003 | JWT access tokens carry `scope` alongside `user_id`/`role`. | Must | Every issued access token includes a `scope` claim identifying the hotel or hotel-group the bearer may act within, sufficient for a downstream check to authorize without a DB round trip. | TRULE-AUTH-005 |
...
| TREQ-AUTH-008 | No rate-limiting/CAPTCHA is introduced at the application layer. | Must (already satisfied) | Login/signup/password-reset remain unthrottled at this layer; edge-level (Nginx/Cloudflare) rate limiting is a separate, out-of-module concern. | TRULE-AUTH-002 |
...
| RULE-AUTH-009 | Any `logAudit` call from this module | `actor_role` is normalized to an uppercase `UserRole` token or stored as `null` (as in `logout`, which always passes `null`, `service.ts:173`). | `logAudit` is a shared `BaseService` method, not owned solely by this module (`state-audit-log`, `authoritative_writer: UNKNOWN`). | `unassigned (SYNC-001)`; `lib/base-service.ts:7-31` |
...
| TRULE-AUTH-005 | Any access-token issuance | The token embeds `scope` alongside `user_id`/`role` so a downstream authorization check never needs a DB round trip to establish "which hotels may this bearer act within." | None. | PIVOT §5.3:170 |
...
- `state-audit-log` (`AuditLog` model, `schema.prisma:494-516`) — written via the shared
  `BaseService.logAudit` by every module including this one; `authoritative_writer: UNKNOWN`
  (`DEPENDENCY_GRAPH.yaml:498,519-520`). Not resolved here.
...
runs globally ahead of this router (`routes/v1/index.ts:20`); five of the seven routes additionally
require `authMiddleware` per-route (`routes.ts:12-14`). Envelope: `{status:"success", data,
...
`docs/03-modules/notifications/MODULE_SPEC.md` Dependencies section, which already lists
`backend-auth` as an expected-but-not-yet-existing future producer) — no such call site exists
today (verified: `modules/auth/service.ts` imports nothing from `modules/notifications`).
...
| `permissions-middleware` (owned `unassigned`, distinct contract, NOT this module) | 11 other modules gate role/permission/hotel-scope checks with it; `backend-auth` itself does NOT consume it | in-process function import | baseline/UNKNOWN | Same typed-error pattern; out of this module's ownership |
...
- **No sweep/TTL job exists** to delete rows whose `expires_at` has already passed (consistent
  ...
  `Session` accumulates dead rows unboundedly over the life of the deployment.
...
- `Session.refresh_token` is stored in cleartext (verified: `session.create`/`update` write
  ...
  storage pattern. Recorded as evidence for the Security Reviewer, not a decided finding.
...
(`TREQ-AUTH-008`). `refreshToken`'s `Session.findFirst` filters on `refresh_token` (unique-indexed,
...
| OQ-AUTH-04 | decision | The JWT refresh-secret fallback (`JWT_REFRESH_SECRET ?? JWT_SECRET`, `lib/jwt.ts:37,72`) means both token types share one signing secret whenever the optional env var is unset in a given environment, and `verifyAccessToken` performs no token-`type` discrimination. Whether this is an accepted operational default or must be closed (e.g. making `JWT_REFRESH_SECRET` required, or adding a `type`-claim check to `verifyAccessToken`) is a human/security decision. Recorded as evidence for the Security Reviewer per this task's framing, not characterized as a defect here. | `lib/jwt.ts:27-33,37,53-68,70-89` | human/unassigned | **OPEN** |
| OQ-AUTH-05 | decision | `resetPassword`'s current-state behavior — `{email,new_password}` body only, no reset token, no email ever sent, public `POST /password-reset` with no `authMiddleware`, silent no-op on unknown/inactive/deleted email, unconditional overwrite + full session revocation on match — is recorded neutrally as evidence throughout this spec (`REQ-AUTH-007`, `RULE-AUTH-004`, Interfaces, Failure/Security). Its severity is for the Security Reviewer to independently assess; this document does not decide it. | `modules/auth/routes.ts:11`; `modules/auth/service.ts:197-212` | human/unassigned | **OPEN — required security disposition before `MIG-GAP-AUTH-05` implementation planning** |
| OQ-AUTH-06 | decision | `checkHotelAccess`'s blanket bypass for `admin`/`manager`/`checker` (`middleware/permissions.ts:105`) means a `manager` today has NO enforced one-hotel scope anywhere in the request pipeline via this middleware. The bypass is attributed in code/test comments to "PATCH-04 §4c," traceable to `docs/legacy/api/API_SPEC_V1_PATCH_V2.md` — per this repository's Source-of-Truth policy, `docs/legacy/` content is historical evidence only, not a current authoritative decision. Whether the target one-hotel Manager scope (`TREQ-AUTH-002`) is implemented by removing this bypass or by a different scope-check layered on top is unstated implementation direction (appropriately out of this spec's scope); the CURRENT absence of manager-level hotel-scoping is recorded as a security-relevant evidenced fact for independent review. | `middleware/permissions.ts:103-108`; `__tests__/rbac.test.ts:92,100`; `docs/legacy/rbac/AUTHORITY_CHAIN_RECONCILIATION_REPORT.md:154-158` (non-authoritative, historical) | human/unassigned | **OPEN** |
...
| OQ-AUTH-11 | decision | Neither CONFIRMED §25's three GDPR retention tiers (6mo shift-coords / 5yr general / 6yr payroll-tax) nor any other authority document explicitly names `User`, `Session`, or `AuditLog` (this module's write surface) under a tier. Same open-question class already carried for `Notification` in the sibling notifications spec (`OQ-NOTIF-02`), tracked here as its own item since it concerns a different set of models. | CONFIRMED §25 (silent on these models); `schema.prisma:117-169,494-516` | human/unassigned | **OPEN — required disposition before G8 Release Readiness** |
...
| OQ-AUTH-13 | decision | The exact string token for the new Regional Manager role (`regional_manager`? `REGIONAL_MANAGER`? something else) is unstated by either authority document — CONFIRMED §1/PIVOT §4.1 use the prose "Regional Manager" throughout, never a code-shaped identifier. This module's existing convention is uppercase-enum/lowercase-claim (`UserRole.MANAGER` ↔ `role:'manager'`); an implementer would need this decided before schema/token work begins. | CONFIRMED §1; PIVOT §4.1, §5.4 (prose only, no identifier) | human/unassigned | **OPEN** |
...
- **MODULE_REGISTRY.yaml:** set `specification` for `backend-auth`
  (`MODULE_REGISTRY.yaml:40`) from `UNKNOWN` → `SPEC-AUTH-001@0.1.0 (REVIEW)`. Do not alter
...
- **DECISION_INDEX.md:** reference ADR-003 (modular monolith) and ADR-008 (source-of-truth
  ...
  but `OQ-AUTH-02` (MFA storage model), `OQ-AUTH-06` (hotel-scope bypass reconciliation), and
  `OQ-AUTH-13` (Regional Manager role-token spelling) are flagged as candidates for a Decision
  Record if/when M1 implementation planning for this module begins — proposed, not created here.
