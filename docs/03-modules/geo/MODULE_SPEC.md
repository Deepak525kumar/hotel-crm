...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-GEO-001 / 0.1.3` |
| Status | `FROZEN` |
...
**Implemented shape (OD-GEO-010, 2026-08-10):** as built, `IF-GEO-DISTANCE-CHECK` is an in-process
method, `GeoService.verifyGeofence(workerId, input, actorRole, actorIp?, attendanceId?)`
(`backend/src/modules/geo/service.ts`), confirming this document's own "likely in-process" guess
above. The optional trailing `attendanceId` is supplied only by `AttendanceService`'s own
`checkIn()`/`checkOut()` call sites (`backend/src/modules/attendance/service.ts`) — never derived
from client input — and is persisted on the created `WorkerGeoCheckin` row, surfaced read-only as
`GeoCheckinDto.attendance_id: string | null` and as an optional `attendance_id` filter on
`GET /geo/checkins` (`ListCheckinsQuerySchema`). `CheckinSchema`, the public HTTP request body for
the worker-facing standalone `POST /geo/checkins` endpoint, intentionally has no `attendance_id`
field.
...
| OD-GEO-010 | decision-required | The frontend's Attendance-detail view needed to surface the geofence-verification events tied to one shift, but `WorkerGeoCheckin` (OD-GEO-002's `backend-geo`-owned table) had no link back to `state-attendance`'s `Attendance` row — the only prior association was a client-side worker_id+hotel_id+time-window heuristic, which could misattribute a geo event to the wrong shift when a worker had two overlapping shifts at the same hotel. **RESOLVED 2026-08-10 (commissioning human, PR #407 follow-up):** add a nullable `attendance_id` column to `WorkerGeoCheckin` (FK to `Attendance.id`, `onDelete: SetNull` — the geo audit trail's own 6-month retention lifecycle must not be truncated by the Attendance row's lifecycle), set only by `AttendanceService.checkIn()`/`checkOut()` when they call `GeoService.verifyGeofence()` (never client-suppliable — `CheckinSchema`, the public HTTP request body for the worker-facing standalone "Verify Location" endpoint, deliberately gained no `attendance_id` field, preserving TREQ-GEO-004's self-scoped-checkin provenance discipline). Null for that standalone endpoint, which has no attendance context. `GET /geo/checkins` gained a matching optional `attendance_id` query filter (still hotel/worker RBAC-scoped as before); `GeoCheckinDto` gained `attendance_id: string \| null`. This is additive only — no existing requirement, rule, or interface content is changed, and pre-existing rows retain `attendance_id: null` (no backfill). | `backend/prisma/schema.prisma` (`WorkerGeoCheckin.attendance_id`); `backend/src/modules/geo/{types,service}.ts`; `backend/src/modules/attendance/service.ts` (both `verifyGeofence()` call sites) | human | **Resolved** (2026-08-10) |
...
| 0.1.3 | 2026-08-10 | **Amendment — new Open Decision raised and resolved same-session (`OD-GEO-010`), additive schema/interface change, no prior requirement/rule/interface text removed or reinterpreted.** The Attendance module's detail view needed to show geofence-verification events for one specific shift; the only prior association between `WorkerGeoCheckin` (OD-GEO-002's `backend-geo`-owned table) and `state-attendance` was a client-side worker_id+hotel_id+time-window heuristic (introduced in PR #407), which could misattribute an event under overlapping shifts. Resolved by adding a nullable `attendance_id` FK column to `WorkerGeoCheckin` (`onDelete: SetNull`, preserving the geo audit trail's independent 6-month retention lifecycle — TREQ-GEO-005/RULE-GEO-004 unaffected), set only by `AttendanceService`'s own `checkIn()`/`checkOut()` calls into `GeoService.verifyGeofence()`, never client-suppliable (the public `POST /geo/checkins` request body, `CheckinSchema`, gained no such field — TREQ-GEO-004's self-scoped-checkin provenance discipline is unchanged). `GeoCheckinDto` gained `attendance_id: string \| null`; `GET /geo/checkins` gained a matching optional filter, still fully RBAC-scoped as before (OD-GEO-005 unaffected — this exposes an FK, not raw coordinates). Interfaces and Contracts section updated with the implemented `IF-GEO-DISTANCE-CHECK` shape. Purely additive: no backfill, pre-existing rows read `attendance_id: null`. | — (new decision `OD-GEO-010`, resolved same-session; no prior finding reopened) | Commissioning human, 2026-08-10 (PR #407 follow-up) |
