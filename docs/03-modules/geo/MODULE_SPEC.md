...
| Approved by / at | **FROZEN at G2 Specification Freeze on 2026-07-27 by the commissioning human, per `GD-14`** (Geofencing/location model — Decided, option (a): hotel coordinates as `Hotel` columns, `backend-geo`-owned worker-coordinate retention, fail-closed, admin-only coordinate entry, computed-distance-only visibility for admin/manager, geofence pass/fail audit-logged). This resolves Architecture's own v0.1.1 freeze condition ("`OD-GEO-001/002` acceptable as open at REVIEW status, must resolve before FROZEN") plus `OD-GEO-003/004/005/007`, all now Decided. All five G4 dimensions were already `PASS_WITH_ACTIONS`/`PASS`, zero Critical/High, unaffected by this decision. Remaining open items (`OD-GEO-006` performance budgets, `OD-GEO-009` GPS-spoofing countermeasure — explicitly deferred, not silently omitted) are implementation/release prerequisites reviewed by G8, NOT freeze blockers, per the `SPEC-DOCUMENTS-001`/`GD-16` precedent. `OD-GEO-008` is a citation correction, not a decision item. See `docs/implementation/GOVERNANCE_DECISIONS_REQUIRED.md` GD-14 for the full decision record. |
