...
| `TREQ-009` marking a current/future day sick or vacation ATOMICALLY auto-cancels any same-day assignment (single transaction); manager notified | PIVOT §7.2; CONFIRMED §22 | Confirmed authority | Target; unbuilt (MIG-GAP-10) |
...
| TREQ-009 | Marking a current or future day sick or vacation atomically auto-cancels any existing assignment for that day in a single transaction, and notifies the manager. | Must | Sick/vacation flag + same-day assignment cancellation commit atomically (no state where both hold); manager notified. | TRULE-008 |
...
| TRULE-008 | Worker marks a current/future day sick or vacation | Any same-day assignment is auto-cancelled atomically in one transaction; manager notified. | No state where a worker is both on-leave and assigned that day. | PIVOT §7.2; CONFIRMED §22 |
...
No event bus exists (MODULE_REGISTRY `published_events: none-observed` for all three modules).
...
to eligible workers (TREQ-003), "requirement fulfilled" to losers (TREQ-005), sick/vacation manager
notification (TREQ-009), 6h auto-close manager notification (TREQ-006). Channel is push-only (CONFIRMED §18).
...
- **New models** — `CalendarEntry` (per-worker per-day assignment/sick/vacation) and `JobRequest` (broadcast; skills×headcount; 6h auto-close), PIVOT §9.3; plus the daily-exclusivity partial unique index (PIVOT §9.4).
...
| MIG-GAP-10 | No sick/vacation calendar flag; no same-day auto-cancel path | Sick/vacation flag atomically auto-cancels same-day assignment; manager notified (PIVOT §7.2; CONFIRMED §22) — TREQ-009 | 2 |
...
TREQ-009 atomic sick/vacation same-day cancel; TREQ-006 6h auto-close job. Success gates per PIVOT §10/§12.
...
| OQ-02 / OQ-03 | Assignment cancel / request cancel cascade semantics | PARTIALLY RESOLVED-BY-TARGET: sick/vacation atomically auto-cancels same-day assignment (TREQ-009/TRULE-008; PIVOT §7.2). Broader broadcast slot reopen semantics are a target design detail for M2; no marketplace PARTIALLY_FILLED in target. |
