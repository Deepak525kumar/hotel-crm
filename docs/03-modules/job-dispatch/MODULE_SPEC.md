...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-JOB-DISPATCH-001 / 0.3.4` |
| Status | `FROZEN` |
...
| `TREQ-003` eligibility = matching skill ∧ free that day; only those workers notified | PIVOT §5.5; CONFIRMED §13 | Confirmed authority | **Eligibility computation Built, Epic 9 PR 9.7** (`getBroadcastEligibility()`, reuses `isWorkerFreeOnDay()`/`ACTIVE_ASSIGNMENT_STATUSES` from PR 9.6 and `listEligibleWorkerIds()` from roster-scope.ts); the "only those workers notified" delivery half remains unbuilt, PR 9.8 (MIG-GAP-04 delivery half still open) |
| `TREQ-004` first-accept wins; tie-break = earliest server-received timestamp; optimistic concurrency (version column + transactional conditional update), per `ADR-057` — no Redis mutex | PIVOT §5.5, §7.3; CONFIRMED §13; `ADR-057` | Confirmed authority | Target; unbuilt (MIG-GAP-06). PR 9.7 added `JobRequestSkillSlot.confirmed_count` (default 0, unused by 9.7's own logic) as a per-skill optimistic-concurrency target for this requirement's future implementation, per architecture review preceding PR 9.7 |
...
| MIG-GAP-04 | Marketplace `WorkRequest` (publish + apply) is the only fan-out (`work-requests/service.ts:222-250`) | Repurpose as broadcast `JobRequest` (skill×headcount; eligibility skill ∧ free; "requirement fulfilled") (PIVOT §7.3, §9.1; CONFIRMED §13) — TREQ-002/003/005 | 2 — **partially closed, Epic 9 PR 9.7**: `raiseBroadcast()` + `JobRequestSkillSlot` close the skill×headcount persistence half (TREQ-002) and `getBroadcastEligibility()` closes the eligibility-computation half (TREQ-003); the "requirement fulfilled" notification half (TREQ-005) and the notify-eligible-workers delivery half (TREQ-003's other half) remain open, PR 9.8/9.9 |
| MIG-GAP-05 | `position` is free text (`schema.prisma:236-260`; `CreateWorkRequestSchema`) | Skills constrained to enum {Cleaner, Public Service, Kitchen Dishwasher, Waiter} (CONFIRMED §4) — TREQ-010 | 2 — **closed for the broadcast path, Epic 9 PR 9.7** (`JobRequestSkillSlot.skill SkillTag`); the legacy marketplace `position` free-text field is intentionally left in place (still live until `TREQ-011`'s removal), so this gap's marketplace-facing evidence path is superseded rather than literally removed — see PR 9.7's architecture review finding AR-9.7-02 |
