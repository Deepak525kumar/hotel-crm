...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-JOB-DISPATCH-001 / 0.3.5` |
| Status | `FROZEN` |
...
| MIG-GAP-04 | Marketplace `WorkRequest` (publish + apply) is the only fan-out (`work-requests/service.ts:222-250`) | Repurpose as broadcast `JobRequest` (skill×headcount; eligibility skill ∧ free; "requirement fulfilled") (PIVOT §7.3, §9.1; CONFIRMED §13) — TREQ-002/003/005 | 2 — **further closed, Epic 9 PR 9.8**: `raiseBroadcast()` + `JobRequestSkillSlot` close the skill×headcount persistence half (TREQ-002, PR 9.7); `getBroadcastEligibility()` closes the eligibility-computation half (TREQ-003, PR 9.7); `enqueueBroadcastNotifications()` now closes the notify-eligible-workers delivery half (TREQ-003, PR 9.8). Only the "requirement fulfilled" notification half (TREQ-005) remains open, PR 9.9. |
