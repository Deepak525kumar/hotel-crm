...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-USERS-001 / 0.1.1` |
| Status | `REVIEW` |
...
| Repository revision | `5a81b65a9ede77382d8ae65cbaa220100111cfc3` (`5a81b65`, current `HEAD` at this correction pass; includes `HOTFIX-AUTH-001`/`002`/`003` and `SYNC-020`'s knowledge-layer rebind; worktree clean — `git status --porcelain` empty) — supersedes the v0.1.0 authoring revision `fc698582b771f76989e4f398074f88a111a5c4f8` (`fc69858`) for every current-state citation re-verified in this pass |
| Approved by / at | — (G2 freeze requires the named human approver; not yet approved). The v0.1.0 High consistency finding and Critical security finding are both resolved (mechanical ID-collision fix applied in-document; root-cause code fix landed via `HOTFIX-AUTH-001`) and no longer block G2 on their own. G2 remains blocked on standing human-authority items unaffected by this pass: `SYNC-001` module-ownership assignment (`OQ-USERS-07`, High), the `state-user` dual-writer decision (`OQ-USERS-04`, High), the 5-role/Regional-Manager model (`OQ-USERS-01`), the cross-hotel read-surface Risk Assessment (`OQ-USERS-05`, Medium, routed not authorized), and the GDPR PII-nulling design (`OQ-USERS-06`) — see Risks, Assumptions, and Open Decisions. Workflow termination state for this candidate: correction cycle resumed and completed for every author-fixable finding; **Blocked** on reserved human authority for G2 freeze (`LOOP_CONTROL.md` §4), not Success/Failure. See `SYNC-016`/`SYNC-017`/`SYNC-020` (`.claude/knowledge/SYNC_STATE.yaml`) and `SIR-GLOB-012`/`SIR-USERS-*` (`.claude/governance/SPECIFICATION_ISSUES_REGISTER.md`). |
| Supersedes | v0.1.0 (this document). First specification for `backend-users`; `.claude/knowledge/MODULE_REGISTRY.yaml:57` recorded `specification: UNKNOWN` prior to this pass. |
...
| `REQ-USERS-025` (evidence; renumbered from the colliding `REQ-USERS-003` by this v0.1.1 consistency fix, `SIR-USERS-010`) `UserRole` enum has exactly 4 values: WORKER, CHECKER, MANAGER, ADMIN | `backend/prisma/schema.prisma:22-27` | Current repository | Confirmed current-state; **gap vs 5-role target (`OQ-USERS-01`)** |
| `REQ-USERS-026` (evidence; renumbered from `REQ-USERS-004`) `User` is indexed on `role`, `is_active`, `deleted_at` | `backend/prisma/schema.prisma:152-154` | Current repository | Confirmed current-state |
...
| `REQ-USERS-032` (evidence; renumbered from `REQ-USERS-010`) The JWT `permissions` claim is a **snapshot** of `ROLE_PERMISSIONS[role]` taken at token-issuance time (login/signup/refresh), not re-derived on every request | `backend/src/modules/auth/service.ts:37-42,88-93,143-148`; `backend/src/lib/jwt.ts:5-12,27-33,44-51` | Current repository | Confirmed current-state |
...
| `REQ-USERS-019` `state-user` (the `User` model) has two writers: `backend-auth` (create at signup, `service.ts:25`; `password_hash` update at password-reset, `service.ts:205`; identity-field update at self-service `updateProfile`, `service.ts:218`) and `backend-users` (this module's full CRUD); `authoritative_writer: UNKNOWN` | `.claude/knowledge/DEPENDENCY_GRAPH.yaml:437-443` (records the aggregate line list `auth/service.ts:25,205,218` without per-line gloss; this spec independently verified and attributes each line above); `users/service.ts:104,143,175` (create/update/delete Prisma calls — consistent with the graph, re-verified against `HEAD` `5a81b65`, up from `93,132,164` at v0.1.0's `fc69858`, shifted by the intervening `HOTFIX-AUTH-003` guard insertion) | Generated (repo-derived) | Confirmed current-state; already tracked platform-wide as `SYNC-005`/`SIR-GLOB-006` — **disclosed, not resolved (`OQ-USERS-04`)** |
...
| `REQ-USERS-021` The confirmed target role model has exactly 5 roles (Staff/Worker, Checker, Hotel Manager, Regional Manager, Admin); no Supervisor role; Regional Manager is new | CRR §1 (lines 13-24, specifically 15,19) | Authoritative (business) | Confirmed target; **gap vs `REQ-USERS-025` (`OQ-USERS-01`)** |
...
| `OQ-USERS-04` | Ownership / decision | **`state-user` shared-write (`SYNC-005`).** `User` is written by both `backend-auth` (signup create `service.ts:25`; password-reset update `service.ts:205`; self-service profile update `service.ts:218`) and `backend-users` (full CRUD); `authoritative_writer: UNKNOWN`. Already tracked platform-wide (`SIR-GLOB-006`); disclosed here from this module's side, not resolved. No coordination mechanism (transaction, lock, event) exists between the two writers. | `.claude/knowledge/DEPENDENCY_GRAPH.yaml:437-443`; `auth/service.ts:25,205,218`; `service.ts:104,143,175` | Human/Architecture | **Open — cross-repository, requires a single-authoritative-writer decision** |
