...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-ATT-001 / 0.2.2` |
| Status | `FROZEN` (Amended (Correction, v0.2.1→0.2.2) 2026-07-28, per `GD-21`/`ADR-059`) |
...
| Repository revision | Module code base described: `cca1d29bcf21f9f8b8fe0f08d61350e6e31dc205` (`cca1d29`) through v0.2.1, confirmed unchanged by every G4 reviewer up to that point. **v0.2.2 (this revision) reflects a real code change**, applied 2026-07-28 per `GD-21`'s two consistency corrections: `attendance/service.ts:162` (`getById` role guard aligned to `list`/`update`'s `{admin,manager,checker}`) and `attendance/service.ts` (`checkIn`/`update` audit calls now populate `old_values`/`new_values` via the extended `BaseService.logAudit`, `backend/src/lib/base-service.ts`). No other behavior changed. |
...
| 0.2.2 | 2026-07-28 | **Amended (Correction), per `GD-21`/`ADR-059`.** `OQ-ATT-07`/`SIR-ATT-007` (attendance automation) RESOLVED: automated reminders and automatic ABSENT/NO_SHOW marking ratified as the permanent target architecture, implementation timing explicitly deferred (destination-vs-journey split, matching `ADR-054`/`ADR-053`/`ADR-038`) — no code authorized for automation itself. Two further items resolved as consistency corrections, applied directly (code change, not merely documentation): `OQ-01` (`getById`'s role guard aligned to `list`/`update`'s `{admin,manager,checker}`, `attendance/service.ts:162`) and `OQ-11` (`BaseService.logAudit` extended platform-wide with optional `old_values`/`new_values`; attendance's `checkIn`/`update` now populate them). Four stale narrative passages describing the pre-fix `getById` asymmetry as current fact (Terminology note, `REQ-010`, `RULE-008`, `GET /attendance/:id` interface row) corrected to match. FROZEN status retained. | `OQ-ATT-07`/`SIR-ATT-007` RESOLVED (`ADR-059`); `OQ-01`/`SIR-ATT-001` and `OQ-11`/`SIR-ATT-012` RESOLVED (consistency corrections) | Commissioning human (2026-07-28, Governance Resolution workflow, `GD-21`) |
