...
4. **Contract generation & presentation (triggering, not execution):** triggering generation of a pre-filled contract PDF from Personalfragebogen data (incl. the 1-year fixed term and 6-month probation clause) and coordinating its availability for download/print for hand signing (CRR §9; PDD §7.1). **RESOLVED `ADR-012` (Accepted, 2026-07-12), correction applied 2026-07-28:** contract generation, templates, and metadata are owned and executed by `backend-hr`/`SPEC-HR-001`; Onboarding triggers/coordinates this step as a consumer via `backend-hr`'s interfaces, mirroring item 3's Chatbot invocation-not-execution pattern — it does not own or execute the generation logic itself.
5. **Hand-signed contract capture (triggering, not execution):** coordinating acceptance of the scanned/photographed signed paper contract (stored in S3 (EU)) and the manager's confirmation that it is "signed & valid"; the system does not capture, verify, or auto-detect the signature (CRR §9; PDD §5.7, §7.1). **RESOLVED `ADR-012`, correction applied 2026-07-28:** contract confirmation-capture, versioning, and storage are owned and executed by `backend-hr`/`SPEC-HR-001`; Onboarding coordinates this step as a consumer, it does not own or execute it.
...
- **Contract ownership:** **RESOLVED `ADR-012` (Accepted, 2026-07-12), correction applied 2026-07-28:** "Contracts module" is not an independent module — it is an alias for `backend-hr`/`SPEC-HR-001`, which owns the complete contract capability: template, versioning, generation, metadata, amendments, signatures, and manager confirmation, plus storage of the manager-confirmed hand-signed contract scan (S3, EU). Onboarding coordinates/triggers generation, presentation, and scan-upload as a `backend-hr` consumer via `IF-HR-*` interfaces; it executes none of this capability itself.
...
7. **Contract generated & presented** — A pre-filled contract PDF (incl. 1-year fixed term and 6-month probation clause) is generated from Personalfragebogen data and made available to download/print for hand signing. *(Execution ownership: `backend-hr`/`SPEC-HR-001`, per `ADR-012`; Onboarding triggers this step.)*
8. **Contract signed & confirmed** — Both parties sign the paper contract by hand, in person; the scan is uploaded (stored in S3 (EU)) and the **manager marks it "signed & valid."** Account is now activated and the 1-year expiry clock starts. Until this point the contract carries a **"contract pending signature"** status. *(Execution ownership: `backend-hr`/`SPEC-HR-001`, per `ADR-012`; Onboarding coordinates this step.)*
