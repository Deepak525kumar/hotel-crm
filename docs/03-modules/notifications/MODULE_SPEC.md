...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-NOTIF-001 / 0.3.1` |
| Status | `FROZEN` (Amended (Correction, v0.3.0→0.3.1) 2026-07-28, per `GD-09`/`ADR-033` — see Decision Integration note below and Change Log 0.3.0/0.3.1) |
| Owner | `unassigned (SYNC-001, human authority required)` |
| Authors / reviewers | Author: Module Author agent. Reviewers: G4 complete — architecture, dependency, consistency, security, and performance reviewers all returned `PASS_WITH_ACTIONS`, zero Critical/High findings, reused unchanged for this freeze. Author-fixable corrections applied in `0.1.1`; see Review and Change Log. |
| Repository revision | `efde2a9f0393cccc2168cd1d2d89ea9cbc21033a` (HEAD at G4 completion; unchanged since) |
| Approved by / at | FROZEN at G2 Specification Freeze on 2026-07-20 by the commissioning human (this session's standing authorization to freeze each spec once its G4 gate is clean), reusing the existing G4 evidence — zero Critical/High findings across all five dimensions. `OQ-NOTIF-01..09` and `SYNC-001` are implementation/release prerequisites reviewed by G8, not freeze blockers, per the `SPEC-AUTH-001`/`SPEC-ATT-001`/`SPEC-USERS-001`/`SPEC-CALENDAR-001`/`SPEC-EMP-001`/`SPEC-CRM-001` precedent. `OQ-NOTIF-01` specifically blocks *target-state implementation planning* for TREQ-002/TREQ-012 (per its own text), not G2 freeze. |
...
## Decision Integration — `ADR-029` (GD-01, 2026-07-23)

> **Forward-note, Decision-Integration class.** The project owner ratified governance decision **GD-01**
> (Notification dispatch & delivery model) as **`ADR-029` — Transactional Outbox + dedicated Worker
> runtime**. `ADR-029` is the authority; this note integrates its dispositions into the frozen spec
> without rewriting the reverse-specified `[CURRENT STATE]` or confirmed `[TARGET STATE]` requirement
> text (those remain the record of code and confirmed intent as of freeze). The affected open decisions
> are marked RESOLVED in the Risks table below.

**What ADR-029 decides (see `docs/14-governance/architecture-decisions/ADR-029-...`):**

1. **Two concepts stay separate.** `Notification` = the user-facing in-app inbox (existing REST API
   authoritative, unchanged). `OutboxEvent` = an internal delivery queue, never client-exposed.
2. **Transactional write.** A producer's transaction persists, in one commit, its domain change + the
   `Notification` inbox row + one `OutboxEvent` per external delivery. This replaces the fire-and-forget
   `.catch(() => {})` pattern (RULE-003).
3. **Worker runtime.** A dedicated Worker (a separate process over the same monolith codebase) polls
   `OutboxEvent` (default 5s, configurable), delivers via transport handlers, runs the retry lifecycle,
   and hosts scheduled reminder/escalation jobs. It is the canonical async execution runtime — the
...
`OQ-AUTH-01` (`SIR-AUTH-005`). **Amends** the prior "Redis-backed BullMQ" scheduled-job assumption in
§Dependencies (`ASM-NOTIF-02` runtime) — replaced by the PostgreSQL-backed outbox + poll Worker.
**Does NOT resolve:** `OQ-NOTIF-02` (retention — GD-09; note `OutboxEvent` is a *new* record type also
needing a tier), `OQ-NOTIF-03` (`FEATURE_*` flag mismatch), `OQ-NOTIF-05` (cross-module send-authz).
...
| `REQ-004` sendEmail always throws NotImplementedError | `notifications/service.ts:40-42` @efde2a9f | Code | Observed at `efde2a9f`; **DELETED 2026-08-02** (zero call sites, superseded by the Epic 7 EMAIL transport handler, `ADR-029`/`OQ-NOTIF-07`) — see forward-note below |
| `REQ-005` sendPushNotification always throws NotImplementedError | `notifications/service.ts:44-46` @efde2a9f | Code | Observed at `efde2a9f`; **DELETED 2026-08-02** (zero call sites, superseded by the Epic 7 PUSH transport handler, `ADR-029`/`OQ-NOTIF-06`) — see forward-note below |
...
| `REQ-015` markAsRead is the ONLY tested method; sendNotification/getNotifications/sendEmail/sendPushNotification have zero test coverage | `backend/src/__tests__/notifications.test.ts:1-79` @efde2a9f | Test (absence) | Observed at `efde2a9f`; `enqueue`/`registerPushToken` gained test coverage in Epic 7; `sendEmail`/`sendPushNotification` DELETED 2026-08-02, no longer applicable |
...
| sendEmail/sendPushNotification have zero call sites anywhere in the repository | repo-wide grep, no matches @efde2a9f | Code (absence) | Observed at `efde2a9f`; both methods DELETED 2026-08-02 on this evidence — no longer present to have call sites |
...
| notification-service | The in-process TypeScript contract (`notificationService.enqueue/sendNotification/getNotifications/markAsRead/registerPushToken`) imported directly by producer modules — not an HTTP or event-bus contract. `sendEmail`/`sendPushNotification` DELETED 2026-08-02 (dead code; delivery now goes through `enqueue()` + the Platform Worker's transport handlers, `ADR-029`). | `notifications/service.ts` |
...
| ~~`notificationService.sendEmail(email, subject, body)`~~ | — | — | — | — | — | **DELETED 2026-08-02** — dead code (zero call sites), superseded by the Epic 7 `EmailTransportHandler` (`outbox-transport.ts`), dispatched via `notificationService.enqueue()` + the Platform Worker, not this in-process method |
| ~~`notificationService.sendPushNotification(userId, title, body)`~~ | — | — | — | — | — | **DELETED 2026-08-02** — dead code (zero call sites), superseded by the Epic 7 `PushTransportHandler` (`outbox-transport.ts`), dispatched via `notificationService.enqueue()` + the Platform Worker, not this in-process method |
...
| MIG-GAP-11 | `sendPushNotification` unconditionally threw `NotImplementedError` at `efde2a9f`; APNs/FCM credentials declared-unwired at that time. **CLOSED by Epic 7** (`PushTransportHandler`, `outbox-transport.ts`, `ADR-029`/`OQ-NOTIF-06`) — a real APNs/FCM-backed push delivery mechanism now exists, driven by the Platform Worker, not this stub. The stub itself was DELETED 2026-08-02 as dead code (zero call sites even before Epic 7; superseded, never repointed-to). | Push-only delivery is the sole confirmed channel (CONFIRMED §18) — TREQ-011 | M1 — **DONE, Epic 7** |
| MIG-GAP-12 | `sendEmail` unconditionally threw `NotImplementedError` at `efde2a9f`; zero call sites. **CLOSED by Epic 7** (`EmailTransportHandler`, `outbox-transport.ts`, `ADR-029`/`OQ-NOTIF-07`) — a real SendGrid/Resend-backed email delivery mechanism now exists, driven by the Platform Worker, not this stub. The stub itself was DELETED 2026-08-02 as dead code (zero call sites, confirmed by an independent repo-wide trace; HR's separate SMTP payslip mechanism, PIVOT §7.7, is unaffected and remains distinct). | CONFIRMED §18 explicitly excludes email/SMS as notification channels; `sendEmail`'s role is now moot — deleted, not repurposed | **DONE — deleted, `OQ-NOTIF-07`** |
...
| ~~REQ-004/005 stub errors~~ | — | — | **MOOT — both methods DELETED 2026-08-02**, nothing left to test; superseded by Epic 7's `EmailTransportHandler`/`PushTransportHandler`, exercised via `outbox-worker.test.ts` |
...
| 0.3.0 | 2026-07-23 | **Decision-Integration amendment — `ADR-029` (GD-01).** Integrated the project owner's ratified notification dispatch & delivery decision (Transactional Outbox + dedicated Worker runtime) via a new "Decision Integration" forward-note after Document Control; marked `OQ-NOTIF-01` (dispatch half), `OQ-NOTIF-04`, `OQ-NOTIF-06`, `OQ-NOTIF-07`, `OQ-NOTIF-08`, `OQ-NOTIF-09` RESOLVED in the Risks table with pointers to `ADR-029`. No reverse-specified `[CURRENT STATE]` or confirmed `[TARGET STATE]` requirement text was rewritten (code/intent record preserved); the amendment adds decision dispositions only. Amends the prior "Redis-backed BullMQ" scheduled-job assumption (§Dependencies) — superseded by the PostgreSQL-backed outbox + poll Worker. Remaining OPEN: `OQ-NOTIF-02` (retention/GD-09, now also covering the new `OutboxEvent` record type), `OQ-NOTIF-03`, `OQ-NOTIF-05`. Knowledge synchronized in the same pass: `DECISION_INDEX.md` (ADR-029 row), `DEPENDENCY_GRAPH.yaml` (`state-outbox` + `worker` nodes), Specification Issues Register (NOTIF + `SIR-AUTH-005` rows), `IMPLEMENTATION_EXECUTION_PLAN.md` §9 ledger + Epic 7 sequence. | `OQ-NOTIF-01`(dispatch)/`-04`/`-06`/`-07`/`-08`/`-09` resolved by `ADR-029` | Project owner (2026-07-23, GD-01) |
| 0.2.0 | 2026-07-20 | **G2 Specification Freeze.** Frozen at G2 by the commissioning human (standing session authorization to freeze once a spec's G4 gate is clean), reusing the existing G4 evidence without reopening any of the five dimensions — all were `PASS_WITH_ACTIONS` with zero Critical/High at v0.1.1. No content change beyond Document Control (`Status` REVIEW→FROZEN, version 0.1.1→0.2.0, `Approved by / at` freeze record) and this row. Open decisions `OQ-NOTIF-01..09` and owner assignment (`SYNC-001`) are implementation/release prerequisites reviewed by G8, not freeze blockers — `OQ-NOTIF-01` specifically gates *target-state implementation planning* for TREQ-002/TREQ-012 per its own text, distinct from G2 freeze, matching the `SPEC-AUTH-001`/`SPEC-ATT-001`/`SPEC-USERS-001`/`SPEC-CALENDAR-001`/`SPEC-EMP-001`/`SPEC-CRM-001` precedent. Knowledge synchronized in the same pass: `MODULE_REGISTRY.yaml`/`SPECIFICATION_INDEX.yaml` (→ `SPEC-NOTIF-001@0.2.0 (FROZEN)`), `MODULE_MEMORY.yaml` (`ART-MEM-backend-notifications` produced), `SYNC_STATE.yaml`, and the Specification Issues Register. | G2 freeze — no new findings; carries forward v0.1.1 dispositions | Commissioning human (2026-07-20, G2) |
| 0.3.1 | 2026-07-28 | **Amended (Correction), per `GD-09`/`ADR-033`** (GDPR retention-tier assignment, Decided via the Governance Resolution workflow, Option (c): provisional mapping now, tax-advisor sign-off tracked separately). `OQ-NOTIF-02` marked RESOLVED (provisional) — `Notification` assigned Tier 2 (5-year general), pending tax-advisor sign-off (`OD-RETENTION-01`), non-blocking. No other requirement, rule, interface, or open decision touched. FROZEN status retained. | `OQ-NOTIF-02` RESOLVED (provisional, `ADR-033`) | Commissioning human (2026-07-28, Governance Resolution workflow, Decision #2) |
| 0.3.1 (forward-note, recorded not versioned) | 2026-08-02 | **Forward-note per `OQ-NOTIF-06`/`OQ-NOTIF-07`/`ADR-029`** — an independent post-implementation dead-code investigation (triggered by a repository-wide MVP-readiness audit's punch list) re-traced every caller of `sendEmail`/`sendPushNotification` against the now-complete Epic 7 pipeline (`EmailTransportHandler`/`PushTransportHandler` in `outbox-transport.ts`, dispatched by the Platform Worker via `notificationService.enqueue()`) and confirmed, with evidence, that both methods had zero call sites anywhere in the repository — not superseded call-for-call, simply never invoked by any code path, before or after Epic 7. Both stub methods DELETED from `notifications/service.ts` on this evidence. REQ-004/005/015, the two dead Interfaces-table rows, MIG-GAP-11/12, OQ-NOTIF-07's disposition, and the `notification-service` terminology-table row are corrected in place to record the deletion; the broader `[CURRENT STATE]`/`[TARGET STATE]` reverse-specified narrative (already known stale re: Epic 7's build, independent of this change) is NOT rewritten here — out of this correction's bounded scope. No requirement/rule/migration-gap/decision id is renumbered. No version bump — nonsemantic forward-note (`LOOP_CONTROL.md` §7 exemption), mirroring the `ADR-026`/quality-spec and `ADR-029` 0.3.0 forward-note precedents: this records a code deletion justified by an already-resolved decision (`ADR-029`), not a new decision. | None — forward-note only, records dead-code deletion on evidence already implied by `ADR-029`'s own disposition. | — (nonsemantic annotation; no approver action required; G2 freeze status unaffected). |
