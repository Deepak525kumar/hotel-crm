...
| Repository revision | `c47df395f9c7c03252fbe79e765edf183e7959f8` (`c47df39`) — code-truth basis for current-state citations (calendar sources are a route-registered stub, unchanged). Target-state ownership reflects `ADR-021` (Accepted). |
...
| Sick/vacation mark emits `EVT-CAL-SickVacationMarked`; auto-cancel is consumer-side, no Calendar cross-write (`REQ-CAL-T04`) | Integration (event) | Assigned day + mark | Event emitted with (worker, day, kind); Calendar writes no assignment state; assignment owner cancels on consumption |
...
- **`DECISION_INDEX.md`:** `ADR-021` (Accepted) resolves `OD-CAL-01/02/03`. Remaining pending decision records: OD-CAL-04 ("today"/timezone), OD-CAL-06 (notification contract), OD-CAL-07 (manager/RM scope), OD-CAL-08 (interface/event schema), OD-CAL-10 (reception-data disposition).
