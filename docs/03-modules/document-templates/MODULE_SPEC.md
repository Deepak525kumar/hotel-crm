...
- **Retention tier assignment and the deletion sweep.** Owned by `backend-retention`. This module's rows carry no retention-tier classification today (`OD-DOCTPL-04`).
...
**Retention/migration:** the migration is purely additive (7 tables, 4 enums, one FK from `DocumentInstance` to `WorkerDocument`); it was verified byte-identical to Prisma's own `migrate diff` output, and is paired with a `down.sql` that drops all seven tables leaf-first then the four enums. No pre-existing data is touched. No retention tier is assigned to these rows (`OD-DOCTPL-04`).
...
**Deployment prerequisite (must not be missed):** `playwright` is a new backend dependency and requires the Chromium binary (~300 MB) to be installed in every environment that runs this code — `npx playwright install --with-deps chromium`. Any deploy image or CI runner that lacks it will fail at first render, not at startup. This is the single most likely thing to break this module in an environment that otherwise looks healthy.
...
| `OD-DOCTPL-04` | No retention tier is assigned to any of this module's seven tables. | GDPR retention sweep (`backend-retention`) will not touch these rows. | **Open** — must be classified before this data is real. |
