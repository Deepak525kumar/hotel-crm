...
| Field | Value |
|---|---|
| Spec ID / version | `SPEC-ANALYTICS-001 / 0.2.2` |
| Status | `FROZEN` (Amended (Correction, v0.2.1→0.2.2) 2026-07-28, per `GD-13`/`ADR-034`) |
...
| RULE-ANALYTICS-002 | Route-level authorization | All four routes require `requireRole(['admin','manager','regional_manager'])` (fixed on the leaderboard routes 2026-07-17, S0-5; `regional_manager` added 2026-07-26, `SIR-ANLY-015`, `ADR-030` §3 C-31); `/leaderboard/by-hotel/:hotel_id` additionally requires `checkHotelAccess()`, which resolves a Regional Manager's hotel-group scope correctly (`isScopedManagerRole()`, `SIR-AUTH-021`). `/leaderboard` (global) and `/stats` carry no hotel-membership dimension by design (global/platform-wide scope); a Regional Manager's response is narrowed to their own group server-side via `resolveNonAdminScopeFilter()` (`analytics/controller.ts`), not by a client-supplied `hotel_id`. | WORKER/CHECKER rejected with 403 on all four routes. `checkHotelAccess()`'s own admin/manager/checker bypass (`SIR-AUTH-003`) means a manager can still read another hotel's `by-hotel` leaderboard and `hotel-summary.top_workers` — tracked as `OQ-ANALYTICS-12` (Low), not this rule's own defect; this bypass does NOT extend to `regional_manager`, which is scope-checked, not bypassed. | `unassigned (OQ-ANALYTICS-06/SYNC-001)`; `analytics/routes.ts:9-27` |
...
| OQ-ANALYTICS-11 | risk | `backend-analytics` owns no state; 100% of its contract is unversioned, direct cross-module Prisma reads across seven domains owned by five other modules, with no ADR governing this cross-module read-access boundary (Constitution §7, "explicit interfaces; consumers do not reach into private internals"). **RESOLVED `GD-13`/`ADR-034`, 2026-07-28:** direct read-only Prisma reads are ratified as the platform standard for aggregator/reporting modules, subject to a three-point allow-list (read-only; plain query, no re-implementation/bypass of the owning module's own business rules; display/reporting only, never another module's write-path decision source). All seven existing read edges satisfy this criterion — no code change required. A future dedicated read-model interface remains the correct escalation only if a named trigger condition is met (frequent schema-shape breakage, or a confirmed performance bottleneck once `GD-11`'s SLO baseline exists) — neither is met today. | "Ownership and Boundaries" (owns no state, seven `reads-state` edges to five owning modules); "Dependencies" (same seven edges); `ADR-034` | human/architecture (Lead Architect, platform-wide) | **RESOLVED — direct read-only pattern ratified, `ADR-034`; escalation to a read-model interface remains a named, unmet future trigger, not current scope** |
...
| 0.2.2 | 2026-07-28 | **Amended (Correction), per `GD-13`/`ADR-034`** (Cross-module state-read boundary ADR, Decided via the Governance Resolution workflow, Option (c)). `OQ-ANALYTICS-11` marked RESOLVED — direct read-only cross-module Prisma reads ratified as the platform standard for aggregator/reporting modules, subject to a three-point allow-list; all seven existing read edges (corrected from "six" — `state-rooms-completed-entry` was added by `ADR-028` after the open-decision prose was last written, never bumped) satisfy the criterion, no code change required. A future dedicated read-model interface remains a named, unmet escalation trigger, not current scope. No other requirement, rule, interface, or open decision touched. FROZEN status retained. | `OQ-ANALYTICS-11` RESOLVED (`ADR-034`) | Commissioning human (2026-07-28, Governance Resolution workflow, Decision #3) |
| 0.2.1 (forward-note, recorded not versioned) | 2026-08-05 | **Forward-note, Regional Manager V1 — closes a real documentation gap `SIR-ANLY-015` (register) had already recorded but this spec never reflected.** `RULE-ANALYTICS-002` previously said "all four routes require `requireRole(['admin','manager'])`" as a current-state fact — stale since 2026-07-26, when `regional_manager` was added to all four gates and the `analytics:read` permission grant/`checkHotelAccess()` scope resolution were extended in lockstep (`SIR-ANLY-015`, commit `5719e05`; `ADR-030` §3 C-31). This spec had zero mentions of Regional Manager anywhere despite the module being fully RM-aware in code — the widest spec/code inversion found during this synchronization pass, since the underlying fix was already register-tracked and merged well before this correction. Corrected in place at `RULE-ANALYTICS-002` above. No requirement or rule identifier renumbered; no G4 dimension re-run. FROZEN status retained. | `RULE-ANALYTICS-002` corrected to name `regional_manager`; no new decision, a pre-existing gap closed | Lead Architect (2026-08-05, Regional Manager V1 PR6 documentation synchronization) |
