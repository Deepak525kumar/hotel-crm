...
- **Status:** **Proposed.** Authored 2026-07-26 (session `claude/lead-engineer-orchestration-4193hw`) against repository revision `45f6ce2`. This record is **not** ratified: per the `ADR-029`/`ADR-030` lifecycle (Constitution §20 human/product + security authority), a Proposed record is presented to the project owner with options, evidence, and a recommendation, and **only** the owner's explicit decision moves it to Accepted. No PR in §7 is authorized until that ratification.
- **Date:** 2026-07-26
