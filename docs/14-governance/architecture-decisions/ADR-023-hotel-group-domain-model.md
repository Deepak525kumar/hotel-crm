...
1. **`HotelGroup` is a first-class entity**, minimally: `{ id, name, billing_info, regional_manager_user_id }`. Justification: CRR §11's "shared billing" is a group-level property, forcing a table (a derived/virtual grouping cannot hold billing data). One `HotelGroup` has exactly one assigned Regional Manager (`regional_manager_user_id`) — a Group-side cardinality confirmed by **three independent, convergent authorities**: `CONFIRMED_REQUIREMENTS_REGISTER.md` §11 ("grouped under one Regional Manager"), `PIVOT_DESIGN_DOCUMENT.md` §9.1 line 100 ("Hotel Groups aggregate hotels under a Regional Manager"), and `SPEC-CRM-001` `RULE-CRM-08`/`REQ-CRM-007` ("≥1 hotel per group; ≤1 group per hotel; one Regional Manager per group"; acceptance criterion "the group has one Regional Manager"). The `regional_manager_user_id` FK direction (on `HotelGroup`, pointing to the manager) already permits one Regional Manager to be referenced by multiple `HotelGroup` rows — i.e., to manage more than one Hotel Group — while still enforcing exactly one Regional Manager per Hotel Group; no schema change would be needed if a Regional Manager is later confirmed to manage several groups. No authority states or requires the reverse (a Hotel Group having more than one Regional Manager), so this FK shape is already the correct and sufficient model.
...
4. **Employee ↔ `HotelGroup`: explicit FK on the employment record, owned by `backend-hr`/`SPEC-EMP-001`.** There is no separate "Employee" entity in the target model — `SPEC-EMP-001`'s Terminology defines "Employment record" as "the durable per-employee record owned by this module (repurposed retained roster record)," and Owned state lists only the employment record itself. The `hotel_group_id` field therefore lives directly on **the employment record** (`ADR-022` §1: the retired `HotelWorker` roster "is repurposed into the Employee Management employment record"), set at the `Under Review → Active` hire-approval transition (`RULE-EMP-03`) from the approving manager's own `hotel_group_id`. An explicit field is required — CRR §12's boundary cannot be derived from assignment history without circularity (the boundary bounds the very assignments that would need to derive it). This is the minimal target-state analogue of the retired `HotelWorker` per-hotel tie, now at group grain instead of hotel grain, added to the one existing target entity rather than a new one.
...
| `SPEC-CRM-001` | Independently corroborates the Group-side one-Regional-Manager cardinality (`RULE-CRM-08`, `REQ-CRM-007`) and the entity's target ownership (`backend-crm`'s own `IF-CRM-Hotel Group CRUD` interface, Terminology "Hotel Group (Organization)"). This ADR's ownership/cardinality decisions resolve `SPEC-CRM-001`'s own `OD-CRM-01` (data-model existence) alongside `OD-EMP-05`, though authoring that text into `SPEC-CRM-001` is routed to that spec's next revision, not this ADR. |

No blocking contradiction found against any of the six checked authorities.
