...
2. **Every action the chatbot performs is a tool call into an existing, already-approved `IF-*` interface owned by another module.** The chatbot never gains a bespoke backend capability created solely for its own benefit — a tool is a conversational wrapper around a capability the owning module already exposes (or builds for its own reasons, then exposes as a tool).
