...
- The production deploy script runs `npx --no-install prisma migrate deploy` (`deploy.sh:30`), invoked from the `.github/workflows/deploy.yml` deploy job.
...
- Migrations are version-controlled and applied deterministically on deploy via `prisma migrate deploy` (`deploy.sh:30`).
...
| Production deploy runs `npx prisma migrate deploy` | Confirmed | `deploy.sh:30` |
