...
| Field | Value |
|---|---|
| Purpose | A single, exhaustive, per-module inventory of every unresolved governance item (`OD-*`, `OQ-*`, and cross-cutting `SIR-*`/`SYNC-*` rows) across all 16 platform modules, plus a consolidated cross-reference to the genuinely-still-open platform-level `GD-*` decisions (now **6**, across the full `GD-01..23` numbering — see the closing note below for the itemized list). This is **pure inventory** — Phase 2, Step 1–2 of the Governance Finalization roadmap. |
...
| Live status | The Governance Resolution workflow is now underway. **`GD-12` (Decision #1)**, **`GD-09`'s mapping half (Decision #2)**, **`GD-13` (Decision #3)**, **`GD-11` (Decision #4)**, **`GD-10` (Decision #5)**, **`GD-17` (Decision #6)**, **`GD-23` (Decision #7)**, and **`GD-08` (Decision #8)** were all decided 2026-07-28 (`ADR-032`, `ADR-033`, `ADR-034`, `ADR-035`, `ADR-036`, `ADR-037`, `ADR-019`/`ADR-020` ratification, `ADR-038` respectively). This document's per-item rows and Part 2's table have been updated in place to reflect all eight resolutions; every other item remains exactly as open as when this register was first assembled. Three items were found already resolved/closed by prior decisions when GD-10/GD-11/GD-23 were verified: `SIR-CRM-013`/`OD-CRM-13` (`ADR-030`), `SIR-QUAL-005`/`OQ-QUAL-04` (`GD-04`), and `SIR-GLOB-003` (ADR-001..009 ratified 2026-07-15) — none reopened, all corrected in place. Part 2 carries the current count of genuinely-still-open `GD-*` decisions. |
...
| OD-EMP-04 | Offboarding/termination + re-engagement workflow undefined | Product/Human | Open — blocks Deactivated-transition acceptance | Product |
| OD-EMP-05 | Hotel-Group association mechanism | Architecture/Human | Resolved (`ADR-023`) | Architecture |
| OD-EMP-06 | Staff self-edit after onboarding | Product/Human | Open | Product |
...
| OD-EMP-08 | Bulk-import row handling + permission holder | Product/Human | Permission-holder facet Resolved (`ADR-030`); invalid-row/duplicate-handling facet remains Open | Product |
...
| OD-EMP-13 | Job Title value domain (free-text vs. controlled list) | Product/Human | Open | Product |
| OD-EMP-14 | Skill-set governance (administratively editable?) | Product/Human | Open | Product |
...
| OD-CAL-04 | "Today"/timezone + current-vs-future boundary | Product/Architecture/Human | Open (implementation default `Europe/Berlin` applied 2026-07-27, not a formal resolution) | Product |
...
| OD-CAL-07 | Which manager roles may edit the calendar / cross-hotel scope | Product/Human | Open | Product |
...
| OD-CAL-10 | Current-stub semantic drift (`/operations` models reception data, not scheduling) | Product/Architecture/Human | Open — no consumer today, non-breaking to change | Architecture |
| OD-CAL-11 | Auto-cancel ripple to broadcasts (re-open/re-broadcast a freed slot?) | Product/Human | Open | Product |
...
| OQ-05 | Under the calendar model, does EXPECTED-seeding move out of `work-applications`? (= `GD-20`) | human/architecture | Open | Architecture |
| OQ-06 | Fate of `expected_start`/`expected_end` denormalization under calendar-direct-assignment (= `GD-20`) | human/architecture | Open | Architecture |
...
| OQ-ANALYTICS-04 | Status-enum re-validation needed once Job Dispatch pivots (= `GD-20`); internal PIVOT §9.1/§9.3 inconsistency | human/architecture | Open | Architecture |
...
| OQ-ANALYTICS-07 | Sick/vacation counts depend on unbuilt `CalendarEntry`/target Calendar model (= `GD-18`) | human/architecture | Open — blocked on M2 | Architecture |
...
| OD-HR-02 | Current-state code types/routes don't match confirmed target model | Architecture/Human | Open — blocks implementation without a redesign pass | Architecture |
| OD-HR-02b | Personalfragebogen data-source ambiguity for contract pre-fill | Architecture/Human | Open | Architecture |
| OD-HR-03 | Contract lapse/non-renewal/offboarding workflow undefined | Product/Human | Open | Product |
...
| OD-HR-07 | Mechanism for "both parties wish to continue" (contract extension trigger) unspecified | Product/Human | Open | Product |
| OD-HR-08 | Concurrency behavior for contract confirmation unaddressed | Architecture/Human | Open | Architecture |
| OD-HR-09 | No escalation/reminder behavior for an unfulfilled payslip request | Product/Human | Open | Product |
| OD-HR-10 | Current RBAC excludes WORKER/CHECKER from `hr:*`, conflicting with CRR §23's "worker can REQUEST a payslip" | Architecture/Human | Open — high-impact gap | Security |
...
| OD-HR-13 | Hotel Manager could read/write contract/payroll data for workers outside their hotel | Architecture/Human | Partially resolved (write-path scope enforcement now exists, `ADR-030` PR-1); list-route scope-filtering and every not-yet-built interface remain open | Security |
| OD-HR-14 | Malware/content scanning of uploaded contract scans | Architecture/Human | Open — deferred, disclosed | Security |
...
| **GD-15** | HR & Employee-Management module build scope | P2 (post-MVP; large) | `OD-HR-02/03/07/09/10/13/14`, `OD-EMP-04/06/08 (facet)/13/14/16` |
...
| **GD-18** | Calendar module scope (M2) | P3 | `OD-CAL-04/07/10/11` (`OD-CAL-06/08` now resolved at the transport level by `GD-12`; the remaining open Calendar items are timezone/RM-scope/operations-stub-disposition/broadcast-ripple, none of which are transport questions) |
| **GD-19** | Chatbot module scope & LLM safety | P3 | `OD-CHAT-001..022` (`OD-CHAT-023` now resolved by `GD-12`) |
| **GD-20** | Job-Dispatch two-tier calendar+broadcast pivot | P3 (explicitly deferrable) | `MIG-GAP-JOBD-01..12`, `OQ-ATT-05/06`, `OQ-ANALYTICS-04` |
...
**Note:** the original document lists 22 as "remaining open" as of 2026-07-24 (23 total minus GD-01 through GD-23). This session independently confirmed GD-02, 03, 04, 05, 06, 07, 14, 16 are now also decided (and built); **GD-12, GD-09 (mapping half), GD-13, GD-11, GD-10, GD-17, GD-23, and GD-08 were all decided via the Governance Resolution workflow on 2026-07-28** (`ADR-032`, `ADR-033`, `ADR-034`, `ADR-035`, `ADR-036`, `ADR-037`, `ADR-019`/`ADR-020` ratification, `ADR-038`) — leaving **6** genuinely open across the full GD-01..23 numbering: `GD-15` (HR/EMP build scope, itself partly gated on GD-03's own still-open org-chart half), `GD-18` (Calendar M2), `GD-19` (Chatbot scope/LLM safety), `GD-20` (Job-Dispatch pivot), `GD-21` (Attendance automation), `GD-22` (Hotel-Group billing). `backend-retention`'s own remaining open items (`OD-RETENTION-05/10/11/14/15`) are module-level `OD-*` rows, not a standalone `GD-*`, and are not part of this count. `GD-03`'s org-chart/reporting-model half (`OD-EMP-12`, `OQ-AUTH-08`'s data-model half) and `GD-04`'s tiers/warnings/photo sub-decision remain open as named exceptions carved out of those otherwise-decided items — tracked above under `GD-15`/employee-management and quality's `OQ-02/08` respectively, not under a standalone GD row.
