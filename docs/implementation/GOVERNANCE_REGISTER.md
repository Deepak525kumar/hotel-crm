...
4. **Hierarchical Onboarding Gate resolved.** The lack of a formal onboarding mechanism for non-Worker roles and the timing of scope assignment has been fully resolved by `ADR-065` (2026-08-10). All roles now require activation before scope assignment.
...
| OQ-AUTH-08 | Org-chart visibility confirmed but no model/endpoint exists | human/unassigned | **RESOLVED, both facets** — permission half (`ADR-030`, 2026-07-25); data-model half **RESOLVED `GD-03`/`ADR-060`, 2026-07-29** — flat, hotel-scoped, no reporting-tree model; visibility derived from existing hotel/hotel-group scope membership | Product |
...
| OD-CRM-10 | Shared-billing ownership (= `GD-22`) | Product/Architecture/Human | **Resolved by disposition 2026-07-28** — `ADR-023` already settled ownership (`backend-crm`); no new architecture introduced; `billing_info` remains an opaque placeholder pending a concrete requirement | Product |
...
| OD-EMP-12 | Org-chart/reporting-relationship model underlying `REQ-EMP-013` | Product/Human | **RESOLVED `GD-03`/`ADR-060`, 2026-07-29** — flat, hotel-scoped; no explicit `reports_to` FK/reporting-tree model; org-chart visibility (RM+Admin) derived implicitly from existing hotel/hotel-group scope membership (`ADR-023`/`ADR-030`). This is `GD-03`'s remaining half — `GD-03` is now fully resolved, and `ADR-058`'s named eligibility gate for Job-Dispatch (`GD-20`/Epic 9) is satisfied. | Product |
...
| ID | Description | Authority | Status | Category |
|---|---|---|---|---|
| OQ-01 | `getById` privileges only `{admin,manager}` while list/update also privilege `checker` — asymmetry | human/unassigned | **Resolved 2026-07-28 (`GD-21`) — consistency correction, not a governance fork.** `getById` aligned to the same `{admin,manager,checker}` guard already used by `list`/`update` (`attendance/service.ts:162`). | Security |
...
| OQ-07 | Is ABSENT/NO_SHOW automated or manual? (= `GD-21`) | human | **Resolved 2026-07-28 → `ADR-059`.** Automated reminders + automatic ABSENT/NO_SHOW marking ratified as permanent target architecture; implementation timing explicitly deferred (destination-vs-journey split, per `ADR-054`/`ADR-053`/`ADR-038`). Mechanism is the Platform Worker (`ADR-029`/`ADR-057`). Current manual path unaffected. | Product |
...
| OQ-11 | `AuditLog.old_values`/`new_values` never populated (thin `{assignment_id}` payload only) | human/unassigned | **Resolved 2026-07-28 (`GD-21`) — platform-level consistency correction, not attendance-specific.** Root cause was `BaseService.logAudit` itself lacking `old_values`/`new_values` params; extended the shared helper (optional, backward-compatible), wired attendance's `checkIn`/`update` calls to use them. `ADR-016`'s single-shared-writer principle preserved. | Security |
...
**Update (SYNC-073, 2026-08-01 — corrects this section's baseline-era "module unbuilt" framing, does not resolve any row below):** this baseline (`13cf8d6`) predates `backend-retention`'s implementation. `backend/src/modules/retention` now exists (PRs #294-#298, 2026-07-31/08-01) — see `MODULE_REGISTRY.yaml`'s `backend-retention` entry. Every row below remains exactly as open as stated; none is decided by the module's existence.
...
| OD-RETENTION-16 *(new, SYNC-073, 2026-08-01)* | `RetentionLog` has no composite index covering `record_ref` — `IF-RETENTION-CheckEligibility`'s `record_ref`-supplied path is `category_id`-scoped but not sub-indexed within a category (only `@@index([category_id])`/`@@index([tagged_at])`/`@@index([deleted_at])` exist). Surfaced by PR 5's own independent review (commit `14963aa`), never previously tracked in any governance artifact. | human/architecture | Open — non-blocking (zero production categories registered today, unmeasurable); follow-up schema change (`@@index([category_id, record_ref])`) to add before any high-cardinality consumer registers. See `SIR-RETENTION-004`. | Architecture (performance) |
...
| ~~**GD-20**~~ | ~~Job-Dispatch two-tier calendar+broadcast pivot~~ | — | **DECIDED 2026-07-28 → `ADR-054` through `ADR-058`** (5 sub-decisions). Struck through per append-only convention, not removed. Sub-decision 1 → `ADR-054` (two-tier target architecture ratified; marketplace is current, not legacy). Sub-decision 2 → `ADR-055` (broadcast manager-initiated only; no implicit JobRequest reopen). Sub-decision 3 → `ADR-056` (direct WorkerAssignment creation, target model only). Sub-decision 4 → `ADR-057` (Platform Worker + optimistic concurrency; no BullMQ/Redis, evaluated against ADR-035's workload baseline, which did not justify introducing either). Sub-decision 5 → `ADR-058` (existing Phase 1/Phase 2 plan ratified unmodified; architecturally eligible on GD-03, scheduling deferred to implementation planning). Sub-decision 6 (`OQ-05`/`OQ-06` [attendance], `OQ-ANALYTICS-04`) closed as MOOT — mechanically resolved by `ADR-054`/`ADR-056`, no genuine ambiguity remained; one documentation defect (WorkRequest/JobRequest inconsistency in MODULE_SPEC.md) logged to SIR register, not a decision gap. Resolves `MIG-GAP-01..12` (`SIR-JOBD-006`). |
| ~~**GD-21**~~ | ~~Attendance operational automation~~ | — | **DECIDED 2026-07-28 → `ADR-059`** (sole sub-decision: `OQ-ATT-07`). Struck through per append-only convention, not removed. Automated reminders + automatic ABSENT/NO_SHOW marking ratified as permanent target architecture, implementation timing deferred (destination-vs-journey split). Two items resolved as consistency corrections, not governance forks: `OQ-01` (checker `getById` guard aligned to `list`/`update`) and `OQ-11` (`BaseService.logAudit` extended platform-wide with optional `old_values`/`new_values`). |
| ~~**GD-22**~~ | ~~Hotel-Group billing model~~ | — | **RESOLVED BY DISPOSITION 2026-07-28** — struck through per append-only convention, not removed. No genuine architecture fork existed: `ADR-023` already settled ownership (`backend-crm` owns `HotelGroup.billing_info`); CRR gives zero billing mechanics, so none were invented (Constitution §12). `billing_info` remains an opaque, unvalidated-beyond-length placeholder; schema/ownership unchanged. Future billing implementation requires its own new decision grounded in a real requirement. Resolves `OD-CRM-10`/`SIR-CRM-010`. |
...
**Note:** the original document lists 22 as "remaining open" as of 2026-07-24 (23 total minus GD-01 through GD-23). This session independently confirmed GD-02, 03, 04, 05, 06, 07, 14, 16 are now also decided (and built); **GD-12, GD-09 (mapping half), GD-13, GD-11, GD-10, GD-17, GD-23, GD-08, GD-15 (ten sub-decisions), and GD-18 (four sub-decisions) were all decided via the Governance Resolution workflow on 2026-07-28** (`ADR-032`, `ADR-033`, `ADR-034`, `ADR-035`, `ADR-036`, `ADR-037`, `ADR-019`/`ADR-020` ratification, `ADR-038`, `ADR-039`–`ADR-048`, `ADR-049`–`ADR-052`); **`GD-19`'s first sub-decision was also decided the same day** (`ADR-053`), after which the commissioning human explicitly **deferred the remainder of `GD-19` to post-MVP** — a product decision, not an unresolved item awaiting attention. Counting `GD-19` as deferred rather than open, and with `GD-20` (`ADR-054`–`ADR-058`, 2026-07-28), `GD-21` (`ADR-059`, 2026-07-28), and `GD-22` (resolved by disposition, 2026-07-28 — no new architecture) now also decided, **zero remaining standalone `GD-*` decisions** exist across the full GD-01..23 numbering. This is the final MVP governance phase's completion point for the standalone `GD-*` items; `GD-19`'s deferred remainder (post-MVP resumption) and one remaining named residual item below (`OD-HR-02b`) are separately tracked follow-ups outside any decided `GD-*` row — they are not counted in the "zero" above, and a reader should not infer no work remains. `GD-15`'s own resolution left two items genuinely unresolved, not part of `GD-15`'s scope: `OD-HR-02b` (Personalfragebogen data-source ambiguity — `ADR-039` explicitly did not touch it) and `GD-03`'s org-chart/reporting-model half (`OD-EMP-12`, `OQ-AUTH-08`'s data-model half — a standalone item, not folded into `GD-15`). **`GD-03`'s org-chart half is now RESOLVED (`ADR-060`, 2026-07-29)** — flat, hotel-scoped, no reporting-tree model; `GD-03` (5-role model & Regional-Manager authority) is fully decided, and `ADR-058`'s named eligibility gate for Job-Dispatch (`GD-20`/Epic 9) is satisfied. `OD-HR-02b` remains the sole genuinely open named residual item outside any decided `GD-*` row. `GD-18`'s resolution similarly surfaced, but did not resolve, two likely-missing requirements (arrivals count, staffing-demand target — CRR §19-adjacent but not confirmed) flagged for a future Requirements Intake pass; that same real-world planner evidence prompted `OD-CAL-11`'s explicit deferral to `GD-20` for re-broadcast policy. `backend-retention`'s own remaining open items (`OD-RETENTION-05/10/11/14/15`) are module-level `OD-*` rows, not a standalone `GD-*`, and are not part of this count. `GD-04`'s tiers/warnings/photo sub-decision remains open as a named exception carved out of that otherwise-decided item — tracked above under quality's `OQ-02/08`, not under a standalone GD row.
