...
| ID | Status | Summary |
|---|---|---|
| `OD-CHAT-005` | **CLOSED 2026-09-04 by `ADR-073` (Accepted). No longer blocks G2.** The owner's rule: the assistant's authority is the user's own authority, never more; a Manager may act on a worker inside their own scope precisely because they can already do so by hand. This resolves BOTH previously-open halves (read scope beyond the owning worker; write/initiation scope). Prior status, retained for context: **PARTIALLY RESOLVED.** The MUST-level worker-id-provenance precondition (the calling module must derive the worker id from its own authenticated actor, never client-supplied) is resolved and stated directly in the Interfaces table's Auth columns. **Still open:** (1) read-scope beyond the owning worker — who else (manager, auditor, Compliance) may read a conversation/outcome; (2) write-path/initiation-scope — whether a calling module may legitimately direct a conversation at a worker other than the one whose request triggered it (e.g., manager-initiated on a worker's behalf). |
| `OD-CHAT-006` | **CLOSED 2026-09-08 by `ADR-074` (Accepted). No longer blocks G2 — all three blockers are now closed.** Ratified after a live injection battery: 10/10 attempts contained, with a worker's manifest holding 13 of 20 tools and zero manager-scoped ones. Amended at ratification to require a monitored tripwire that logs injection signals and never blocks, closing the gap that containment was silent. Prior status, retained for context: **ANSWERED by `ADR-074` (Proposed, 2026-09-07).** Prompt injection is treated as UNPREVENTABLE and contained rather than detected: no classifier is relied upon, and the platform instead guarantees a fully compromised model cannot exceed the authority of the person it acts for, with any irreversible action requiring their explicit confirmation of the exact call. Eleven controls are ratified as required, all existing in code. Note `ADR-074` §5 CONSTRAINS `OD-CHAT-008`: transcripts are not replayed and results never return to a model, which is what makes second-order injection structurally impossible — persisting and replaying history would remove that property and needs a compensating control decided in the same change. |
| `OD-CHAT-013` | **RESOLVED 2026-09-04.** Module owner is the **commissioning human / account owner**, assigned by them directly. This item could never be closed by an architecture decision -- `SYNC-001` reserves owner assignment to human authority (Constitution §12) -- which is why it stood open longer than every other item here. Ownership means: approving each tool under `ADR-053` item 4, ratifying `SPEC-CHATBOT-001` at freeze, and holding the `OD-CHAT-006` prompt-injection decision. |
