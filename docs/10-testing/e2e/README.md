...
| 07 | `scenarios/07-frontend-ui-playwright.md` | Browser-driven UI walkthrough (Playwright). **All 9 steps run live 2026-09-02.** Role-gated nav, onboarding checklist, real uploads, client-side guards, submit-for-review, manager review→modal→approve, double-click race, negative nav all confirmed. Closed three stale "known defect" notes (worker submit 403, six-not-seven file inputs, "no assign UI"); found and fixed one real defect (Step 9 — `updateUserRole`'s Manager/RM branches never synced `EmploymentRecord`, leaving a freshly-assigned RM invisible in every user listing including their own) |
...
| 22 | `scenarios/22-zelle-rota-and-everyday-tools.md` | Swap a shift, apply a dictated plan (absences + shifts), withdraw a worker's absence, staffing requests list/cancel, team rooms today, fixing a logged room, own phone number, worker boundary. **Executable:** `backend/scripts/chatbot-e2e-conversations.ts` runs 21 and 22 (manager, worker, regional manager, admin) through the real app, login, consent, live model and database; `frontend/e2e/zelle-live.spec.ts` covers Copy, History and the pre-filled form in a real browser |
| 23 | `scenarios/23-manager-app-auth-and-capability-gating.md` | **Manager app.** Sign-in and role admission (worker/checker sent to a wrong-app screen *by name*); RM sees everything a manager sees plus the org chart; no quality write, no room-log write, no master data for manager/RM — each asserted in the UI **and** at the API; onboarding lockout; archived hotel confers no scope. Records `FEATURE_GD02_MATRIX` state against `SIR-CRM-020` rather than asserting either outcome |
| 24 | `scenarios/24-manager-app-rota-and-operations.md` | **Manager app.** The rota: agenda opens on the *Berlin* day from a device in another timezone; long-press drag moves a placement, verified by a Postgres read; a drop off-strip or on the origin day sends nothing; the accessibility action performs the same move; vacation-without-reason refused; cancelled placements stay visible; requests/broadcasts as one list; attendance verification writes all four fields and cross-scope returns 403 (that route has **no** role gate — the service is the whole boundary) |
| 25 | `scenarios/25-manager-app-people-and-platform.md` | **Manager app.** Review queue scoped, proven with **two** groups; worker approval = one history row; manager approval = two steps, verified by reading `Hotel.manager_user_id`; `role` in a user PUT refused at the boundary; six locales + RTL with no raw keys; 375pt with the width *measured*; airplane mode readable; `PushToken.app = 'MANAGER'` with a real provider send |
| 26 | `scenarios/26-manager-app-dto-contracts-and-history.md` | **Manager app.** The round-3 DTO-contract class: a placement that nests its worker and hotel instead of two cuids; the `/users` page-size cap that made every name on the rota fall back to a cuid; a filter chip (`ASSIGNED`) that is not in the enum and rejected the whole list; a page-size parameter the server silently drops; `worker_name` on an assignment detail that `enrichContext()` declared and never assigned; a blocklist row that names a person, and the User cuid that 404'd every add; a cancellation reason verified in Postgres; and the History tab that was **empty by construction** for every role that could open it, with scope asserted four ways. **Run live 2026-09-23: 10/10** — found two *test-suite* defects in scenario 23 (two steps asserting routes that do not exist, so they 404'd for every caller and proved nothing) |
| 27 | `scenarios/27-manager-app-hr-analytics-and-leaderboards.md` | **Manager app.** The read surface that had no scenario: HR contract list scoping (worker refused outright; a manager with **no scope claim** gets zero rows); analytics that fail closed by **data** (200 with every figure zero) while the hotel-keyed endpoints fail closed by **status** (403) — both safe, and the asymmetry recorded so neither is "fixed" into the other; and the **two** leaderboards, where a worker is refused `/analytics/leaderboard` but allowed `/quality/leaderboard` per `ADR-067`, seeing only their own group and no contact fields. **Run live 2026-09-23** |
| 28 | `scenarios/28-manager-app-creation-and-dispatch-lifecycle.md` | **Manager app.** The write surface: raising a request only for a hotel you hold; why an end time **before** the start is correct (an overnight shift — do not "fix" it); placement refused unless the worker is eligible, which is resolved from the employment record's group and **not** the `HotelWorker` roster (nothing in the API writes that table at all); and the real-path fixture chain that finally made scenario 24's cross-scope attendance step executable. **Run live 2026-09-23** |

**Start with 00. Then 01-22 in order (skipping 08), then the manager-app set 23-28.** Scenarios 21-22 are also automated end to end
