...
export async function recordToolCall(params: {
  ...
  const status =
    params.outcome.status === 'SUCCESS' ? 'SUCCESS' : 'DENIED';
  ...
  try {
    const row = await prisma.chatbotToolCall.create({
      data: {
        ...
        status,
        denial_reason:
          params.outcome.status === 'DENIED' ? params.outcome.denialCode : null,
        duration_ms:
          params.outcome.status === 'SUCCESS' ? params.outcome.durationMs : null,
      },
      select: { id: true },
    });
