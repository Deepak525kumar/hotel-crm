import { CreateRatingSchema } from '../modules/quality/types';
import {
...
describe('inspection checklist (TREQ-005 / MIG-GAP-07)', () => {
  ...
  it('accepts every confirmed item', () => {
    ...
    const parsed = CreateRatingSchema.safeParse({ ...base, criteria_scores });
  ...
  it('rejects an unknown key', () => {
    const parsed = CreateRatingSchema.safeParse({
      ...base,
      criteria_scores: { teleportation: 100 },
    });
    expect(parsed.success).toBe(false);
  });
  ...
  it('still allows omitting the checklist entirely', () => {
    expect(CreateRatingSchema.safeParse(base).success).toBe(true);
  });
});
